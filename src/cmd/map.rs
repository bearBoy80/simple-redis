use super::{extract_args, validate_command, CommandExecutor, Hmget, Set, RESP_OK};
use crate::{
        ...
        match hmap {
            Some(hmap) => {
                ...
                for key in self.fields {
                    let v = hmap.get(&key);
                    match v {
                        Some(v) => data.push(v.value().clone()),
                        _ => data.push("nil".into()),
                    }
                }
                RespArray::new(data,false).into()
            }
            None => RespArray::new([],false).into(),
        }
    }
}
    ...
    fn try_from(value: RespArray) -> Result<Self, Self::Error> {
        ...
        let fields = args
            .filter_map(|arg| match arg {
                RespFrame::BulkString(filed) => {
                    let result = String::from_utf8(filed.0).unwrap();
                    Some(result)
                }
                _ => None,
            })
